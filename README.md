...
- **Recovery** — if Discord refuses a thread's embed, the post is retried with backoff, and the thread is still recorded rather than abandoned. The summary lists those under **Description pending**; re-running `/challenge-fetch` adopts threads it already created (matching by name, archived ones included) and posts the description it still owes, so a retry repairs instead of duplicating. `/challenge-refresh` does the same for challenges already tracked.
